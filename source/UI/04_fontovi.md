...
Класа `Font` имплементира `IDisposable` интерфејс. `Font` објекат графичкој
Windows Forms библиотеци није само .NET Framework објекат - oн представља и
ресурс оперативног система. Ако се ови ресурси не ослободе на време, оперативни
систем може остати без њих, што доводи до грешака или успоравања апликације.
Зато `Font` класа имплементира `IDisposable`, како би се могao експлицитно
ослободити ресурс оперативног система када се заврши са коришћењем фонта.

У овом конкретном примеру, где се нови фонт одмах додељује `Font` својству
контроле, Windows Forms систем ће се углавном побринути за управљање ресурсима
старог фонта. Међутим, ако би креирао много `Font` објеката у петљи за неке
привремене сврхе (нпр. цртање), а не би их додељивао контролама, онда би
требало да експлицитно позовеш методу `Dispose()` на њима, или да користиш
`using` блок.

Нека је креирана форма са три дугмета B, I, U, једном контролом `NumericUpDown`
и једном лабелом. Задатак је да се кликом на дугме B, текст у лабели задебља,
кликом на дугме I текст у лабели укоси, кликом на дугме U текст у лабели подвуче
и променом вредности у `NumericUpDown` контроли текст у лабели повећа или смањи.
Ако корисник поново кликне на дугме B, текст више не треба да буде задебљан, ако
поново кликне на дугме I не треба више да буде укошен и ако поново кликне на дугме
U не треба више да буде подвучен.
...
`new Font(FontFamily family, float emSize, FontStyle style)`. Први конструктор
