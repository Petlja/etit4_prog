...
[Колекције података](07_kolekcije.md) поменуто је да је колекција речник у
...
што значи да има веома ефикасну алгоритамску сложеност $Θ(1)$ за већину
основних операција:

* Додавање новог елемента (`Add`)
* Приступ елементу по кључу (`dictionary[key]` или `TryGetValue`)
* Провера да ли кључ постоји (`ContainsKey`)
* Уклањање елемента по кључу (`Remove`)

С друге стране, због употребе хеш табеле, ова колекција може заузети више
меморије у поређењу са другим структурама података.
...
* `Remove(TKey kljuc)`, уклања елемент са кључем `kljuc` из речника,
...
[Колекције података](07_kolekcije.md) поменуто је да је колекција сортирани
...
колекцију речник, сортирани речник не користи хеш табелу већ једну варијанту
балансираног бинарног стабла (енгл. *Red-Black Tree*), што проузрокује
логаритамску сложеност $O(\log n)$ за операције уметања, брисања и претраге.
Иако има нешто спорије перформансе у односу на речник, ова генеричка колекција
омогућава аутоматско сортирање кључева.
