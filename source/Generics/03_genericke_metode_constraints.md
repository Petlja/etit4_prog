...
О референтним типовима учио си у
...
* структуре без експлицитног конструктора – јер свака структура подразумевано
има конструктор без параметара, али није експлицитно декларисан као `public` и
...
class Program
{
    public static void Ispisivanje<T>(T objekat) where T : IIspis
    {
        Console.WriteLine(objekat.GetType().Name);
    }
...
class Program
{
    ...
    {
        if (velicina is IStruja)
        {
            (velicina as IStruja).IspisiVelicinu();
        }
    }
