...
* Конекциони приступ (connected), и
...
може бити проблем када постоји велики број клијентских апликација које се
...
користити именски простор `System.Data.SqlClient` који садржи специјализоване
класе за рад са SQL Server-ом (као што су `SqlConnection`, `SqlCommand`,
`SqlDataReader` итд).

Класа `SqlConnection` користи се за везу са *SQL Server* базом података, класа
`SqlCommand` користи се за извршавање SQL упита над базом и класа `SqlDataReader`
користи се за секвенцијално читање резултата ред по ред из базе.

## Пример конекционог приступа

Конекциони приступ нећеш често користити у овом поглављу, али такав
приступ свакако има својих предности када је потребна директна интеракција са базом.
...
`Read()` позива ред по ред. Даље, `reader["CompanyName"]` приступа колони
...
затворити – `using` блокови обезбеђују да се ресурси ослободе чак и ако дође до
изузетка.
...
Метода у којој треба да дефинишеш догађај клика на дугме
...
поново се отвара конекција, шаљу се измене, након чега се конекција затвара.

За бесконекциони приступ важи да су мања оптерећења сервера јер су конекције
краткотрајне, али постоји опасност од појаве конфликата ако више корисника
...
ћеш користити именске просторе `System.Data` који садрже основне класе за рад
...
Класа `SqlConnection` се користи за везу са *SQL Server* базом података, класа
`SqlCommand` се користи за извршавање SQL упите над базом и класа `SqlDataAdapter`
се користи као посредник који аутоматски: отвара конекцију, извршава упит, пуни
податке у `DataSet` или `DataTable` и затвара конекцију. `DataTable` и `DataSet`
користе се за чување података у меморији твог рачунара.
...
ослобађање ресурса дешава се аутоматски.
