...
Осим читања података из реда, `DataRow` можеш да користиш и за креирање и додавање
...
и којег су типа. Да би то урадио, потребно је да у `using` блоку додаш једну
...
омогућава креирање приказа (прегледа) података из `DataTable` објекта, без
...
Класе `DataSet`, `DataTable`, `DataRow`, `DataColumn` и `DataView` омогућавају
флексибилан рад са подацима у ADO.NET-у, без сталне везе са базом. Омогућавају
манипулацију, анализу и приказ података у апликацијама на ефикасан и објектно
оријентисан начин. У наредним лекцијама научићеш како се подаци учитавају из
базе, а затим и како се уписују, ажурирају и бришу.
