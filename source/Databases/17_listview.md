...
приказ,
[`ListView`](https://learn.microsoft.com/en-us/dotnet/api/system.windows.forms.listview?view=netframework-4.8)
је много флексибилнија контрола. Она може приказати податке на више начина, од
којих су најчешће коришћени:
...
`Form1_Load` методу тако да у `Tag` својству сваке ставке буде смештен ID
...
Да би изменио податке у неком реду, корисник треба да кликне на тај ред, потом
...
Брисање је најједноставније. Када корисник одабере производ, позива се метода
...
аутоматизовани приступ који смо имали код `DataGridView`-а када је повезан са
`DataTable` објектом.
